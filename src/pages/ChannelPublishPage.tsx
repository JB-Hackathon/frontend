...
import previewImage1 from '@/assets/data/image_2_1.jpg';
import previewImage2 from '@/assets/data/image_2_2.jpg';
import previewImage3 from '@/assets/data/image3.jpg';
...
export default function ChannelPublishPage() {
  ...
  const [selectedChannelIds, setSelectedChannelIds] = useState<Set<string>>(
    () => new Set(data.channels.filter((channel) => channel.checked && !channel.disabled).map((channel) => channel.id)),
  );
  ...
  const scheduledOption = data.scheduleOptions.find((opt) => opt.id === 'scheduled');
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-5xl mx-auto w-full px-6 py-5 space-y-2">
          ...
          {!hasSelectedChannel && (
            <p className="text-xs text-rose-500">게시할 채널을 1개 이상 선택해주세요.</p>
          )}
        </div>
      </div>

      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-6 space-y-5">
        <FlowPathBar from="심의 결과 · 승인 완료" current="채널 게시 라우팅" to="게시 완료 → 내 콘텐츠" />
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-1">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-400">{selectedChannels.length}개 선택됨</span>
            </div>
            <button className="text-xs text-[#1B3A6B] font-medium hover:underline">채널 계정 관리 ›</button>
          </div>
              ...
              return (
                <li
                  ...
                  className={`flex items-center gap-3 py-3.5 transition-colors rounded-lg px-2 -mx-2 ${
                    channel.disabled ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-gray-50'
                  }`}
                >
                  <input
                    type="checkbox"
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${channel.disabled ? 'text-gray-400' : 'text-gray-800'}`}>
                      {channel.name}
                    </p>
                    <p className="text-xs text-gray-400 font-mono truncate">{channel.handle}</p>
                  </div>
                  <span
            ...
            {data.scheduleOptions.map((opt) => (
              <button
                ...
                className={`px-4 py-2 font-medium transition-colors ${
                  scheduleId === opt.id ? 'bg-[#1B3A6B] text-white' : 'text-gray-500 hover:bg-gray-50'
                }`}
              >
                {opt.label}
              </button>
      ...
      {openCardIndex !== null && (
        ...
        >
          <img
            src={CARD_PREVIEW_IMAGES[openCardIndex % CARD_PREVIEW_IMAGES.length]}
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-7 space-y-5 text-center">
            ...
            <div className="space-y-1.5">
              <h3 className="text-base font-bold text-gray-900">
                {isScheduled ? '예약 게시가 등록되었습니다' : '게시가 완료되었습니다'}
              </h3>
