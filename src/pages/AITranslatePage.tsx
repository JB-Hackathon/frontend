...
export default function AITranslatePage() {
  ...
  const active = data.translations.find((t) => t.code === activeCode) ?? data.translations[0];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-6 space-y-5">
        <FlowPathBar from="심의 결과 · 승인 완료" current="AI 다국어 변환" to="변환본 저장 → 채널 게시" />

        {/* 원본 콘텐츠 */}
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
          <h2 className="text-sm font-bold text-gray-800">원본 콘텐츠 · 한국어</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
          <div>
            <h2 className="text-sm font-bold text-gray-800">변환 언어</h2>
            <p className="text-xs text-gray-400 mt-0.5">칩을 눌러 추가/제외</p>
          </div>
          ...
          {active && (
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-gray-900">{active.languageName}</h3>
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-600 border border-emerald-200">
                  {active.reviewBadge}
                </span>
              </div>
